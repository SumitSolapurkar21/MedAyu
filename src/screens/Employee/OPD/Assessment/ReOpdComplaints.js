...
const ReOpdComplaints = () => {
     ...
     const inputChangeHandlerforEdit = (id, field, value) => {
          const updatedData = opdAssessmentforEdit?.map(item =>
               item.id === id ? { ...item, [field]: value } : item
          );
          // Assume you have a function to set the state of opdAssessmentforEdit
          setOpdAssessmentforEdit(updatedData);
     };
     ...
     const FetchMobileOpdAssessmentForEdit = async () => {
          try {
               await axios
                    ...
                    .then(res => {
                         const opdData = res.data.opdcomplaintArray;
     ...
     const _buttonHandler = event => {
          if (event === 'Save') {
               ...
               navigation.navigate('ReOpdPastHistory');
          }
     };
...
export default ReOpdComplaints;
