...
const ConditionAtDischarge = ({route}) => {
    ...
    try {
      await axios
        .post(`${api.baseurl}/FetchPatientSummary`, {
          ...
          ip_no: ip_no,
          condition_status: 'Discharge',
        })
        .then(res => {
          const {status, message, discharge_summary} = res?.data;
          if (status === true) setText(discharge_summary);
...
export default ConditionAtDischarge;
